...
func TestDeleteLocalBranch(t *testing.T) {
	...
	// Create a branch that is already merged (no new commits).
	...
	if err := r.DeleteLocalBranch(dir, "to-delete"); err != nil {
		t.Fatalf("DeleteLocalBranch: %v", err)
	}
}

func TestDeleteLocalBranchError(t *testing.T) {
	r := NewRunner()
	dir := initRepo(t)
	if err := r.DeleteLocalBranch(dir, "no-such-branch"); err == nil {
...
func TestDeleteRemoteBranch(t *testing.T) {
	r := NewRunner()
	// Create a "server" repo (bare) and a client clone of it.
	serverDir := t.TempDir()
	for _, args := range [][]string{
		{"init", "--bare", "-b", "main", serverDir},
	} {
		cmd := exec.Command("git", args...)
		if out, err := cmd.CombinedOutput(); err != nil {
			t.Fatalf("git %v: %v: %s", args, err, out)
	...
	// Create a commit so the clone isn't empty, then push a new branch.
	...
	for _, args := range [][]string{
		...
		{"commit", "-m", "init"},
		{"push", "origin", "main"},
		{"checkout", "-b", "feature-to-delete"},
		{"push", "origin", "feature-to-delete"},
	} {
		cmd := exec.Command("git", args...)
		cmd.Dir = clientDir
		if out, err := cmd.CombinedOutput(); err != nil {
	...
	if err := r.DeleteRemoteBranch(clientDir, "feature-to-delete"); err != nil {
		t.Fatalf("DeleteRemoteBranch: %v", err)
	}
}

func TestDeleteRemoteBranchError(t *testing.T) {
	r := NewRunner()
	dir := initRepo(t) // no remote
	if err := r.DeleteRemoteBranch(dir, "no-such"); err == nil {
		t.Error("expected error from DeleteRemoteBranch with no remote")
	}
}
